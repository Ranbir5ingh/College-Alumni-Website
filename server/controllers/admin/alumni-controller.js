...
    const filter = {};
    
    // Apply filter only if the query value is valid
    if (isQueryValid(batch)) filter.batch = batch;
    if (isQueryValid(department)) filter.department = department;
    if (isQueryValid(yearOfPassing)) filter.yearOfPassing = parseInt(yearOfPassing);
    if (isQueryValid(accountStatus)) filter.accountStatus = accountStatus;
    
